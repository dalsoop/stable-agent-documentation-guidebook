...
- "AGENTS.md는 효율을 위한 파일이다". 근거 [4]가 에이전트 하나만 다뤘고 정확성을 측정하지 않았으며, 다른 연구 [3]과 비교할 수 없는 조건이었다.
- R-003 "규칙은 에이전트가 실제로 틀렸을 때 고친다" (2026-09-25). "기능 설명을 덧붙이지 않는다"는 절반은 R-002와 P-001이 이미 말한다. 나머지 절반의 근거는 규칙 수정과 코드 수정을 분리하지 못한 연구 [9] 하나뿐이었고, 기계로 검사할 수 있는 적용 방법도 없었다.
- P-003 "저장소 개요가 에이전트의 작업 성공률을 높인다는 근거는 없다" (2026-09-25). 근거 등급이 낮음이라 원칙 층 조건을 만족하지 못했다. 같은 근거 [3] [5]와 한계는 R-002에 있다.
