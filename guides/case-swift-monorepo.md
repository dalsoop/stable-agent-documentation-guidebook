...
This is a record of one repository that followed [new-project.md](new-project.md). The repository is private, so generic names replace its name and the names of its tools. The rules for guides in [DESIGN.md](../DESIGN.md) §2 tell how to use a case.
...
- Each feature change added sentences to the README, ARCHITECTURE and the instruction file. The same text was in three documents. No document described the folder layout.
- Each app also had its own instruction file, and these files repeated the same conventions.
...
|---|---|---|---|
| Instruction file | R-001, R-002 | One root file with four rules. Each rule ends with "(Reason: ...)". The file has 17 lines and cites R-001 and R-002 | `doctor` fails if an instruction file is outside the root |
| Instruction files of apps | R-002 | Six instruction files of apps were deleted. The usage of each app moved to its README | Same as above |
...
| Rules with no check | Step 7 | The "Check status" section of ARCHITECTURE lists rules such as dependency order and template use | None: a person verifies them |
...
- **No measurement of effect.** Nobody compared the growth rate or the ratio of rule deletions before and after. Thus this case cannot test the falsifiers of R-001 or R-002.
