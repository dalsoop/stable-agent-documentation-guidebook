...
- Takes: an analysis of the lifetime of 247,694 instructions in 1,867 repositories. Instruction files grow by addition, and people rarely delete old instructions. The prescription to give a reason with each instruction.
...
- Takes: the use of available checkers of documentation consistency on instruction files, with no new tool.
...
- Takes: 2,303 instruction files from 1,925 repositories. The files change through frequent small additions. This repository uses the dataset as its sample of instruction files.
...
- Takes: a procedure. State each disagreement so that a test can resolve it, and agree before the test to accept the result. This is the source of falsifiers.

**[16]** Z. Zhou, T. Zhou, R. Jia, J. May, "How Language Models Process Negation," arXiv:2605.03052, 2026. https://arxiv.org/abs/2605.03052
- Takes: language models can process negation, but shortcuts frequently cause them to miss it. This is the reason for positive instruction files.
- Limits: the study did not test instruction files.
...
- Takes: terms for the removal of unnecessary text from instructions, which are single source of truth, duplication, sediment, no-op and relevance. One leading term for each concept.
...
- Takes: the writing standard for English. Its rules for sentence length, verb forms, noun clusters and one meaning for each word.
...
- Takes: the writing standard for Korean. Complete sentences, no omitted particles or endings, precise Sino-Korean words, no metaphors in place of plain words, and no em dash.
- Limits: guidance from a practitioner with no empirical evaluation. The author wrote it for the output of a coding assistant. The check verifies only the em dash rule.
